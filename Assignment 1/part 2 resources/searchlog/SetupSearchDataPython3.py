...
googaccessdict = defaultdict(int)  # this is a dictionary that will store the number of google-related page accesses by each
                                   # student who clicked on a google-related page at least once.
...
for line in f:
    ...
    if line.find('google') > -1:
        if lognum in googaccessdict: googaccessdict[lognum] += 1
        else: googaccessdict[lognum] = 1
...
with open('output.csv', 'w') as o:   # open up the output file
    ...
    writer.writerow(["LogID", "NumPages", "WikiAccesses", "GoogAccesses", "NumWords"])
    ...
    for log in sorted(pageaccessdict):  # iterate through the log entries (by their 'log' key).
        ...
        if log in googaccessdict:               # now check if there is an entry in the googaccess
            goog_accesses = googaccessdict[log] # dictionary for this student.  If there is, the
        else:                                   # next entry will be that number, otherwise it will be 0.
            goog_accesses = 0
        ...
        row = [log, page_accesses, wiki_accesses, goog_accesses, words_copied]
